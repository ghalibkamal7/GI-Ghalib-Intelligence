...
import { X, ChevronLeft, ChevronRight, Shuffle, Sparkles } from "lucide-react";
...
function Flashcards({ isOpen, onClose }) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setCards([{ front: "Could not generate cards", back: "Please try again." }]);
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="p-6">
            <div className="flex gap-2 mb-6">
              ...
              <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                ...
                className="px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white text-sm font-medium flex items-center gap-1.5">
                <Sparkles size={14} /> {loading ? "Generating..." : "Generate"}
              </motion.button>
            </div>

            {loading && <div className="space-y-3">{[1,2,3].map((i) => <div key={i} className="h-12 rounded-xl shimmer" />)}</div>}
                  ...
                  <div className={`flashcard-inner ${flipped ? "flipped" : ""}`}>
                    <div className="flashcard-front bg-gradient-to-br from-indigo-900/60 to-purple-900/60 border border-indigo-500/20">
                      <div className="text-center">
                        ...
                        <p className="text-slate-600 text-xs mt-3">Tap to reveal</p>
                      </div>
                    </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { setFlipped(false); next(); }}
                      className="px-4 py-2 rounded-xl bg-red-500/15 hover:bg-red-500/25 text-red-400 text-sm">Again</button>
                    <button onClick={markMastered}
                      className="px-4 py-2 rounded-xl bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-400 text-sm">✓ Got it</button>
                  </div>
