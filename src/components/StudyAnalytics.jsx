...
import { X, TrendingUp, MessageSquare, Clock, Flame } from "lucide-react";
...
function StudyAnalytics({ isOpen, onClose, chats, messages }) {
  ...
  useEffect(() => {
    ...
    setStats({ msgsByDay, totalMsgs, userMsgs, totalChats, topics });
  }, [chats, messages, isOpen]);
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            {[
              ...
              { icon: <TrendingUp size={16} />, label: "Questions Asked", value: stats?.userMsgs || 0, color: "purple" },
            ...
            <div className="flex items-end gap-2 h-28">
              {stats?.msgsByDay?.map((d, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <motion.div
                    initial={{ height: 0 }}
                    animate={{ height: `${bar(d.count, maxCount)}%` }}
